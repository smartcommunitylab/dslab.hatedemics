  ...
  methods: {
    updateInfo: function () {
      ...
      for (let userID of removed) {
        promises.push(API.dialogs.removeUserFromProject(this.id, userID))
      }
      for (let userID of this.editProjectUserSelect) {
        promises.push(API.dialogs.assignUserToProject(this.id, userID, this.editProjectAdminList.includes(userID)))
      }
    ...
    updateUsers: function () {
      ...
      API.login.getUsers().then(function (data) {
        API.dialogs.getProjectByID(self.id).then(function (prData) {
