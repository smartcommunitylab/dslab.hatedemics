...
const selectedLocaleObj = computed(
  () => locales.find((l) => l.code === selected.value) ?? null
);
...
<template>
  <div class="locale-switcher">
    ...
    >
      ...
      <template #selection>
        <span class="flag-only"
          >{{ selectedLocaleObj?.flag }} {{ selectedLocaleObj?.label }}</span
        >
      </template>
      ...
      <template #item="{ item, props }">
        <v-list-item v-bind="props" @click="selectCode((item as any).raw.code)">
          <v-list-item-title
            >{{ (item as any).raw.flag }} {{ (item as any).raw.label }}</v-list-item-title
          >
        </v-list-item>
      </template>
    </v-select>
