...
import highlightable from "@/components/HighlightTable.vue";
...
export default {
  ...
  mounted: function () {
    ...
    let promises = [];
    promises.push(API.dialogs.getTaskInfo(this.projectID, this.taskID));
    if (this.annotationID) {
      promises.push(
        API.dialogs.getAnnotation(this.projectID, this.taskID, this.annotationID)
      );
    } else if (this.annotationParent) {
      promises.push(
        API.dialogs.getAnnotation(this.projectID, this.taskID, this.annotationParent)
      );
    }
    ...
    Promise.all(promises).then(function (result) {
      ...
      if (vueThis.annotationID || vueThis.annotationParent) {
        ...
        if (
          vueThis.annotation_data === undefined ||
          vueThis.annotation_data.length === 0
        ) {
          vueThis.annotation_data = [];
        }
      }
      ...
      if (vueThis.annotation_data.length > 0) {
        let lastSpeaker =
          vueThis.annotation_data[vueThis.annotation_data.length - 1].speaker;
        vueThis.selectedActorForChoice =
          vueThis.actors[
            (vueThis.actorsIDs[lastSpeaker] + 1) % vueThis.actors.length
          ].label;
      } else {
  ...
  methods: {
    ...
    addByChoice: function (data) {
      ...
      this.selectedActorForChoice = this.actors[
        (this.actorsIDs[data.speaker] + 1) % this.actors.length
      ].label;
      ...
      if (this.annotationID) {
        API.dialogs
          ...
          .then(function () {
            ...
            vueThis.$router.push({
              name: "tasks",
              params: { projectID: vueThis.projectID },
            });
          })
    ...
    scrollToPos: function () {
      ...
      if (top < preOffset || top > winHeight - height) {
        let to =
          document.getElementById("high-file-content").scrollTop - (preOffset - top);
        document
          .getElementById("high-file-content")
      ...
      <v-col cols="6">
        ...
        <div v-if="selectedFile <= 0 || loadingFile" class="empty-div">
          <div v-if="selectedFile === -1">
            ...
            <v-text-field
              :label="t('dialog.documentName')"
              required
              v-model="externalGround.name"
            />
            <v-text-field
              :label="t('dialog.externalLink')"
              v-model="externalGround.link"
            />
            <v-textarea
              :label="t('dialog.text')"
              rows="2"
        ...
        <v-container fluid id="dialogue-div" v-if="!loadingData">
            ...
            <template v-if="taskInfo.inside_type === 'choice'">
              ...
              <v-col class="text-center">
                <v-tooltip>
                  ...
                  <div
                    class="tooltip-html"
                    v-html="t('dialog.tooltip.addDynamicTurn')"
                  ></div>
                </v-tooltip>
              </v-col>
            </template>
          ...
          >
                  ...
                  >
                    <template #prepend>
                      <v-tooltip>
                        ...
                        <div
                          class="tooltip-html"
                          v-html="t('dialog.tooltip.addRoundUp')"
                        ></div>
                      </v-tooltip>
                      <v-tooltip>
                        <template v-slot:activator="{ props }">
                        ...
                        <div
                          class="tooltip-html"
                          v-html="t('dialog.tooltip.addRoundDown')"
                        ></div>
                      </v-tooltip>
                      <v-tooltip>
                        <template v-slot:activator="{ props }">
                    ...
                    <template #append>
                      <v-tooltip>
                        ...
                        <div
                          class="tooltip-html"
                          v-html="t('dialog.tooltip.delete')"
                        ></div>
                      </v-tooltip>
                    </template>
                  </v-select>
            ...
            <v-col cols="5" xl="4">
              <v-icon
                v-if="
                  round.ground.length === 0 &&
                  !actorsWithGround.has(annotation_data[index]?.speaker)
                "
